...
舵机驱动（GPIO38 + JS6620 + 15T/50T 内齿圆盘减速）**v1.2 完成**——`stable_sextant` 驱动舵机让 M3 跟踪声源，**feed-forward 补偿**消除闭环振荡。
