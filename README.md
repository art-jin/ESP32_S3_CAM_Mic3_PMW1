...
**已跑通并标定**。在 30–50 cm 距离、语音强度下，4 个非盲区位置的平均方位误差 **< ±10°**；所有方向都能正确归到对应的 60° 扇区（或降级到 2-mic 半平面）。
...
舵机驱动（GPIO38 + JS6620 + 15T/50T 内齿圆盘减速）**v1.1 完成**——`stable_sextant` 驱动舵机让 M3 跟踪声源，**feed-forward 补偿**消除闭环振荡。

**性能**（v1.1，feed-forward 启用）：

| 测试位置 | 响应时间 | 最终 servo | 状态 |
|---|---|---|---|
| 6 点钟（home）| 不动 | 0° | ✓ 完美稳定（swing=0°）|
| 6→7 点钟切换 | **8.7 秒** | **+20°** | ✓ 锁定，零反弹 |
| 6→5 点钟切换 | ~30 秒 | -16° | ⚠ 方向对，未完全到位 |
| 3 / 9 点钟（超范围）| 抑制 | 保持上次 | ✓ 不振荡 |

**可跟踪弧**：M3 在 **5:20 → 6:40 点钟**之间（±20°，约 1 小时 20 分钟弧）。

**说明**：5 点钟比 7 点钟慢，因为 M3 + M1 的 L/R 折叠程度在两侧不对称。这是 3 麦阵列的几何特性，不是 bug。
