...
舵机驱动（GPIO38 + JS6620 + 15T/50T 内齿圆盘减速）**Phase 1-3 已完成**——`stable_sextant` 驱动舵机让 M3 跟踪声源。机械覆盖 ±20°（软限位，避开机械止位震动），M3 能在 5:20 → 6:40 点钟之间微调跟踪。Phase 4 UART 调试因硬件限制（CH343 单向）未完成。开发细节见 `SERVO_PLAN.md`。
